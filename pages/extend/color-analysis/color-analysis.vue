<template>
	<view class="container">
		...
		<view class="tui-colors-container" v-if="colors.length>0">
			<view v-for="(item,index) in colors" :key="index" class="tui-color" :style="{backgroundColor:item}" @tap="copyColor" :data-color="item">{{ item }}</view>
		</view>
		<view class="tui-colors-container" style="width:94%" v-if="rbgColors.length>0">
			<view v-for="(item,index) in rbgColors" :key="index" class="tui-color" :style="{backgroundColor:`rgb(${item})`}" @tap="copyColor" :data-color="`rgb(${item})`">
				<view>RGB</view>
				{{ item }}
			</view>
		</view>
		<view class="tui-btn-box"><tui-button type="green" shape="circle" @click="setMaxColors">set maxColors=4</tui-button></view>
	</view>
</template>
